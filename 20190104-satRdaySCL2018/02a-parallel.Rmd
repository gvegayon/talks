...
date: "Department of Preventive Medicine<br>University of Southern California<br>4 de Enero, 2019"
output:
...
*   Un paquete de R mantenido por el Core Team (viene con R)

*   Permite implementar paralelismo explícito, es decir, el usuario controla de
    manera directa que paralelizar.
    
*   En términos sencillos, paraleliza procesos a través de ejecución de instrucciones
    de R en procesos de R lanzados en paralelo (múltiples sesiones de R).
...
ans0 <- parSapply(cl, 1:nnodes, function(x) runif(1e3))
str(ans0)
```
...
ans1 <- parSapply(cl, 1:nnodes, function(x) runif(1e3))
...
# 3. ENVIANDO LA LLAMADA (BABY)
ans0 <- do.call(cbind, parLapply(cl, 1:nnodes, function(x) {
...
clusterSetRNGStream(cl, 123)
ans1 <- do.call(cbind, parLapply(cl, 1:nnodes, function(x) runif(nsims)))
...
# 3. ENVIANDO LA LLAMADA (BABY)
ans0 <- do.call(cbind, mclapply(1:nnodes, function(x) runif(nsims)))
```
...
set.seed(123) 
ans1 <- do.call(cbind, mclapply(1:nnodes, function(x) runif(nsims)))
